...
export default function AdminPraise() {
    const [posts, setPosts] = useState([]);
    const [contentCategory, setContentCategory] = useState('All');

    const getPosts = async() => {
        const result = await axios(`http://localhost:3001/api/posts/getKategoriesVrsta/1`);
    ...
    return (
        <div>
            <Header isAdmin={true} page={'adminpraise'}/>
            ...
            {
                    contentCategory !== "All"?
                    posts.map((p, index) => p.vrsta === contentCategory && <PostItem category={'pohvala'} key={index} post={p}/>):
                    posts.map((p, index) => <PostItem category={'pohvala'} key={index} post={p}/>)
            }
            </div>
        </div>
    )
